use libipld_raw_cbor::RawCborCodec;
use libipld_core::{
    cid::Cid,
    codec::References,
...
fn roundtrip_with_cid() {
    ...
    let ipld: Ipld = RawCborCodec.decode(&input).unwrap();
    let bytes = RawCborCodec.encode(&ipld).unwrap().to_vec();
...
fn invalid_cid_prefix() {
    ...
    let input = hex::decode(input).unwrap();
    let _: Ipld = RawCborCodec.decode(&input).unwrap();
}
...
fn skip() {
    ...
    RawCborCodec.skip(&mut r).unwrap();
    assert_eq!(r.position(), 9);
    RawCborCodec.skip(&mut r).unwrap();
    assert_eq!(r.position(), 18);
    assert!(RawCborCodec.skip(&mut r).is_err());
}
...
fn ignored_any() {
    ...
    let input = hex::decode(input).unwrap();
    let mut r = Cursor::new(&input);
    let _x: IgnoredAny = Decode::decode(RawCborCodec, &mut r).unwrap();
    assert_eq!(r.position(), 9);
    let _x: IgnoredAny = Decode::decode(RawCborCodec, &mut r).unwrap();
    assert_eq!(r.position(), 18);
    let r: result::Result<IgnoredAny, _> = Decode::decode(RawCborCodec, &mut r);
    assert!(r.is_err());
}
...
fn raw_value() {
    ...
    let input = hex::decode(input).unwrap();
    let mut r = Cursor::new(&input);
    let raw: RawValue<RawCborCodec> = Decode::decode(RawCborCodec, &mut r).unwrap();
    assert_eq!(r.position(), 9);
    assert_eq!(raw.as_ref(), &hex::decode("a163666f6fd82a5800").unwrap());
    let raw: RawValue<RawCborCodec> = Decode::decode(RawCborCodec, &mut r).unwrap();
    assert_eq!(r.position(), 18);
    assert_eq!(raw.as_ref(), &hex::decode("a163666f6fd82a5800").unwrap());
    let r: result::Result<RawValue<RawCborCodec>, _> = Decode::decode(RawCborCodec, &mut r);
    assert!(r.is_err());
}
...
fn indefinite_length_skip() {
    ...
    assert!(IgnoredAny::decode(RawCborCodec, &mut r).is_ok());
...
fn indefinite_length_refs() {
    ...
    let mut r = Cursor::new(&data);
    assert!(<Ipld as References<RawCborCodec>>::references(RawCborCodec, &mut r, &mut refs).is_ok());
    assert_eq!(
        refs[0],
...
fn test_assert_roundtrip() {
    assert_roundtrip(RawCborCodec, &1u64, &Ipld::Integer(2));
}
